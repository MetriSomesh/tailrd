...
TECH_DICTIONARY = {
    ...
    'dart', 'elixir', 'haskell', 'clojure', 'sql', 'html', 'css', 'sass', 'less',
    'bash', 'shell', 'powershell',

    # Frontend frameworks
    'react', 'reactjs', 'react.js', 'angular', 'angularjs', 'vue', 'vuejs', 'vue.js',
    'svelte', 'next.js', 'nextjs', 'nuxt', 'nuxtjs', 'gatsby', 'remix',
    'tailwindcss', 'tailwind', 'bootstrap', 'material-ui', 'mui', 'chakra',

    # Backend frameworks
    'node.js', 'nodejs', 'express', 'expressjs', 'fastapi', 'flask', 'django',
    'spring', 'spring boot', 'asp.net', '.net', 'rails', 'ruby on rails',
    'laravel', 'gin', 'fiber', 'actix', 'nest.js', 'nestjs', 'koa',
    ...
    'langchain', 'llm', 'llms', 'large language model', 'large language models',
    'gpt', 'openai', 'anthropic', 'hugging face', 'huggingface', 'transformers',
    'rag', 'retrieval augmented generation', 'vector database', 'embeddings',
    'fine-tuning', 'prompt engineering', 'agents', 'agentic', 'multi-agent',
    'guardrails', 'evals', 'benchmarks',

    # Databases
    'postgresql', 'postgres', 'mysql', 'mongodb', 'redis', 'elasticsearch',
    'dynamodb', 'cassandra', 'sqlite', 'neo4j', 'pinecone', 'weaviate',
    'chromadb', 'pgvector', 'supabase', 'firebase', 'firestore', 'prisma',
    'sequelize', 'sqlalchemy', 'typeorm',

    # Cloud & Infrastructure
    'aws', 'amazon web services', 'gcp', 'google cloud', 'azure', 'heroku',
    'vercel', 'netlify', 'digital ocean', 'cloudflare',
    'ec2', 'ecs', 'lambda', 's3', 'rds', 'sqs', 'sns', 'cloudformation',
    'terraform', 'pulumi', 'cdk',

    # DevOps & Tools
    'docker', 'kubernetes', 'k8s', 'helm', 'jenkins', 'github actions',
    'gitlab ci', 'circleci', 'ci/cd', 'ci cd', 'ansible', 'nginx', 'apache',

    # Version control
    'git', 'github', 'gitlab', 'bitbucket', 'svn',

    # Data & Messaging
    'kafka', 'rabbitmq', 'celery', 'airflow', 'spark', 'hadoop', 'flink',
    'pandas', 'numpy', 'dbt', 'snowflake', 'bigquery', 'redshift', 'etl',

    # API & Protocols
    'rest', 'rest api', 'rest apis', 'restful', 'graphql', 'grpc', 'websocket',
    'websockets', 'soap', 'openapi', 'swagger',

    # Testing
    'jest', 'pytest', 'unittest', 'mocha', 'cypress', 'selenium', 'playwright',
    'testing', 'unit testing', 'integration testing', 'e2e', 'tdd', 'bdd',

    # Concepts & Methodologies
    'microservices', 'monolith', 'event-driven', 'serverless', 'distributed systems',
    'system design', 'design patterns', 'solid', 'clean architecture',
    'domain-driven design', 'ddd', 'cqrs', 'event sourcing',
    'agile', 'scrum', 'kanban', 'sdlc', 'devops', 'sre',
    'oop', 'oops', 'object-oriented', 'object oriented', 'functional programming',
    'data structures', 'algorithms', 'problem solving', 'problem-solving',
    'concurrency', 'multithreading', 'async', 'asynchronous',
    ...
    # Security
    'oauth', 'jwt', 'authentication', 'authorization', 'rbac', 'encryption',
    'ssl', 'tls', 'https', 'security', 'penetration testing', 'owasp',
}
...
def extract_sections(jd_text):
    ...
    section_headers = [
        ...
        'about us', 'about the team', 'about the role', 'the role',
        'what you will do', "what you'll own", 'what we are looking for',
        "what we're looking for", 'what you bring',
        'nice to have', 'preferred', 'bonus', 'job info', 'benefits',
        'who you are', 'your impact', 'key responsibilities',
        'who thrives here', 'how to apply', 'culture', 'values',
    ]
...
def extract_responsibilities(jd_text):
    ...
    skip_patterns = ['thrives', 'culture', 'values', 'benefits', 'perks',
                     'how to apply', 'about us', 'about the company',
                     'don\'t see', 'open application', 'what to include']
    ...
    for key in sections:
        ...
        if any(skip in key_lower for skip in skip_patterns):
            continue
        if any(kw in key_lower for kw in ['responsibilit', 'what you will do',
                                            'what you\'ll own', 'your impact',
                                            'key responsibilit', 'the role',
                                            'what we\'re looking', 'looking for',
                                            'qualif', 'requirement']):
...
def _is_culture_statement(text):
    ...
    culture_labels = [
        'high ownership', 'speed with depth', 'directness', 'comfort with ambiguity',
        'raise the bar', 'high-agency', 'high agency', 'ego', 'no ego',
        'move fast', 'bias for action', 'default to action', 'low ego',
        'radical candor', 'radical transparency', 'first principles',
        'growth mindset', 'intellectual curiosity', 'humble', 'hungry',
        'smart', 'grit', 'resilience', 'passion', 'passionate',
    ]
    ...
    if len(text) < 80:
        ...
        if not has_tech:
            ...
            culture_signals = {'ownership', 'mindset', 'agency', 'bar', 'ego',
                             'pace', 'depth', 'directness', 'candor', 'grit',
                             'humble', 'hungry', 'passion', 'curiosity',
                             'resilience', 'empathy', 'transparency'}
...
def extract_required_qualifications(jd_text):
    ...
    for key in sections:
        key_lower = key.lower()
        if any(kw in key_lower for kw in ['qualif', 'requirement', 'skills',
                                           'what we are looking', 'what you bring',
                                           'who you are']):
