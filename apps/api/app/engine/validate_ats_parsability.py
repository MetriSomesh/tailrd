...
def validate_parsability(docx_path, resume_json_path):
    ...
    if not extracted_text.strip():
        return {
            ...
            "checks": [],
            "warnings": [
                "The DOCX appears empty or uses non-extractable formatting (text boxes, images)"
            ],
        }
    ...
    checks.append(check_content_presence(extracted_text, immutable["name"], "Candidate Name"))
        ...
        for category, items in skills.items():
            if isinstance(items, list):
                for skill in items[:3]:  # Check first 3 per category
                    checks.append(
                        check_content_presence(extracted_text, skill, f"Skill ({category})")
                    )
    elif isinstance(skills, list):
        for skill in skills[:10]:  # Check first 10
            checks.append(check_content_presence(extracted_text, skill, "Skill"))

    # 5. Check experience bullets
    for i, exp in enumerate(editable.get("experience", [])):
        for j, bullet in enumerate(exp.get("bullets", [])):
            checks.append(
                check_content_presence(extracted_text, bullet, f"Experience[{i}] Bullet[{j}]")
            )

    # 6. Check project content
    for i, proj in enumerate(editable.get("projects", [])):
        checks.append(
            check_content_presence(extracted_text, proj.get("title", ""), f"Project[{i}] Title")
        )
        ...
        for line in desc.split("\n"):
            ...
            if line and len(line) > 15:
                checks.append(
                    check_content_presence(extracted_text, line, f"Project[{i}] Description")
                )
    ...
    for section in expected_sections:
        checks.append(
            check_content_presence(extracted_text, section, f"Section Heading: {section}")
        )

    # 8. Check education
    for edu in immutable.get("education", []):
        checks.append(
            check_content_presence(extracted_text, edu.get("degree", ""), "Education Degree")
        )
    ...
    if any(not c["found"] for c in checks if "Skill" in c["label"]):
        warnings.append(
            "Some skills may not be extractable — check if they are in text boxes or images."
        )
    ...
    if extracted_word_count < 100:
        warnings.append(
            f"Very low word count extracted ({extracted_word_count} words). "
